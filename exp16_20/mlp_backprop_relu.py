...
def forward_pass(x, w1, w2, w3, b1, b2, b3):
    ...
    out = relu(np.dot(h2, w3) + b3)
    return h1, h2, out

def backward_pass(x, y, h1, h2, out, w1, w2, w3, b1, b2, b3, learning_rate=0.01):
    out_error = y - out
    out_delta = out_error * relu_derivative(out)
    h2_error = np.dot(out_delta, w3.T)
    h2_delta = h2_error * relu_derivative(h2)
    h1_error = np.dot(h2_delta, w2.T)
    h1_delta = h1_error * relu_derivative(h1)
